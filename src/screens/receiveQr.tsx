...
import { GradientButton, QRCode } from 'components';
import { useAppSelector, useTranslations } from 'hooks';
import { Icons } from 'theme';
import { Colors } from 'theme/colors';

type ReceiveQrScreenNavigationProp = StackNavigationProp<
  RootStackParamList,
  'ReceiveQr'
>;

type ReceiveQrScreenRouteProp = RouteProp<RootStackParamList, 'ReceiveQr'>;

const ReceiveQr = () => {
  const navigation = useNavigation<ReceiveQrScreenNavigationProp>();
  const { params } = useRoute<ReceiveQrScreenRouteProp>();
  ...
  return (
    <View style={styles.container}>
      <Box w="full" alignItems="center" justifyContent="center" flex={1}>
        <View
          bg={Colors.BG_LIGHT}
          w="90%"
          alignSelf="center"
      ...
      <GradientButton
        onPress={() => navigation.navigate('ReceiveVIP', { token })}
